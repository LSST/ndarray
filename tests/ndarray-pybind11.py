...
import _pybind11_test_mod as testmod
import unittest
...
class TestNumpyPybind11(unittest.TestCase):
    ...
    def testMatrix2d(self):
        ...
        self.assert_((m1 == m2).all())
        self.assert_(testmod.acceptMatrix2d(m2))

    def testArray1(self):
        a1 = testmod.returnArray1()
        a2 = numpy.arange(6, dtype=float)
        self.assert_((a1 == a2).all())
        self.assert_(testmod.acceptArray1(a2))
        a3 = testmod.returnConstArray1()
        self.assert_((a1 == a3).all())
        self.assert_(a3.flags["WRITEABLE"] == False)

    def testArray3(self):
        a1 = testmod.returnArray3()
        a2 = numpy.arange(4*3*2, dtype=float).reshape(4,3,2)
        self.assert_((a1 == a2).all())
        self.assert_(testmod.acceptArray3(a2))
        a3 = testmod.returnConstArray3()
        self.assert_((a1 == a3).all())
        self.assert_(a3.flags["WRITEABLE"] == False)
    ...
    def testOverloads(self):
        ...
        self.assertEqual(testmod.acceptOverload(numpy.zeros((3,3), dtype=float)), 3)
